            ...
            <div className="text-innoq-gray p-8">
                <div className="max-w-6xl mx-auto space-y-8">
                    ...
                    <div className="flex justify-between items-start">
                        ...
                        <div className="bg-[#f7f7f7] p-4 rounded-lg border-2" style={{ borderColor: `${ACCENT_COLOR}33` }}>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                ...
                                <div className="bg-white p-4 rounded border" style={{ borderColor: '#ff4d671a' }}>
                                    <h3 className="text-lg font-mono font-medium mb-3" style={{ color: '#ff4d67' }}>Potential Impact</h3>
                                    <ul className="space-y-2 text-innoq-gray-75">
                                        <li className="flex items-start gap-2">
                        ...
                        <div className="grid md:grid-cols-3 gap-4">
                            {objectives.map(obj => (
                                <div
                                    ...
                                    style={{ borderColor: obj.completed ? '#55cdaf' : `${ACCENT_COLOR}33` }}
                                >
                                    <div className="flex items-center gap-2 mb-2">
                                        {obj.completed ? (
                                            <div className="w-4 h-4 rounded-full bg-innoq-green" />
                                        ) : (
                                            <div className="w-4 h-4 rounded-full border-2" style={{ borderColor: `${ACCENT_COLOR}80` }} />
                                        )}
                                        <h3 className="font-medium font-mono text-innoq-gray">{obj.title}</h3>
                                    </div>
                                    <p className="text-sm text-innoq-gray-75">{obj.description}</p>
                                    {obj.completed && (
                                        <p className="text-sm text-innoq-green mt-2 font-mono">+100 points!</p>
                                    )}
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="grid grid-cols-12 min-h-[400px] border-2 rounded-lg" style={{ borderColor: `${ACCENT_COLOR}33` }}>
                            ...
                            <div className="col-span-3 border-r p-4" style={{ borderColor: `${ACCENT_COLOR}33` }}>
                                <div className="space-y-2">
                                    <button
                                        ...
                                        className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg font-mono transition-all ${selectedFolder === 'inbox' ? 'bg-white' : 'hover:bg-white'
                                            }`}
                                    ...
                                    <button
                                        ...
                                        className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg font-mono transition-all ${selectedFolder === 'trash' ? 'bg-white' : 'hover:bg-white'
                                            }`}
                            ...
                            <div className="col-span-9 divide-y" style={{ borderColor: `${ACCENT_COLOR}1a` }}>
                                {(selectedFolder === 'inbox' ? currentEmails : deletedEmails).map((email) => (
                                    <div key={email.id} className="p-4 hover:bg-white transition-colors">
                                        <div className="flex justify-between items-start mb-2">
                                            <h3 className="font-medium font-mono text-innoq-gray">{email.subject}</h3>
                                            <span className="text-sm text-innoq-gray-75 font-mono">{email.date}</span>
                                        </div>
                                        <div className="flex justify-between items-start">
                                            <div className="font-mono text-sm">
                                                <p className="text-innoq-gray-75">FROM: <span className="text-innoq-gray">{email.from}</span></p>
                                                <p className="text-innoq-gray-75">TO: <span className="text-innoq-gray">{email.to}</span></p>
                                            </div>
                                        </div>
                                        <p className="mt-2 text-sm text-innoq-gray-75">{email.content}</p>
                                    </div>
                                ))}
                                {(selectedFolder === 'inbox' ? userEmails : deletedEmails).length === 0 && (
                                    <div className="p-8 text-center text-innoq-gray-75">
                    ...
                    {showCompose && (
                        <div className="fixed inset-0 bg-white flex items-center justify-center z-50 backdrop-blur-sm">
                            <div className="bg-[#f7f7f7] rounded-lg p-6 w-full max-w-2xl border-2" style={{ borderColor: ACCENT_COLOR }}>
                                <h3 className="text-xl font-semibold font-mono mb-4 flex items-center gap-2" style={{ color: ACCENT_COLOR }}>
                                    <Send className="w-5 h-5" />
                                    <span className="text-innoq-gray-75">[</span> COMPOSE_EMAIL <span className="text-innoq-gray-75">]</span>
                                </h3>
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-white border-2 rounded-lg p-3 text-innoq-gray font-mono focus:outline-none"
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            className="w-full h-64 bg-white border-2 rounded-lg p-3 text-innoq-gray font-mono focus:outline-none"
                                            style={{ borderColor: `${ACCENT_COLOR}4d` }}
                                        />
                                    </div>
                    ...
                    <TerminalSection title="Agent Execution Modes" accentColor={ACCENT_COLOR}>
                        <div className="grid md:grid-cols-3 gap-4">
                            {modes.map((mode) => (
                                <div
                                    key={mode.id}
                                    className={`p-4 rounded-lg border-2 transition-all ${processingMode === mode.id ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white'
                                        }`}
                                    style={{ borderColor: `${ACCENT_COLOR}33`, backgroundColor: '#f7f7f7' }}
                                >
                                    <div className="flex justify-between items-start mb-2">
                                        <h3 className="font-medium font-mono" style={{ color: ACCENT_COLOR }}>{mode.name}</h3>
                                        {mode.warning && (
                                            <FileWarning className="w-4 h-4 text-innoq-apricot" />
                                        )}
                                    </div>
                                    <p className="text-sm text-innoq-gray-75 mb-2">{mode.description}</p>
                                    {mode.warning && (
                                        <p className="text-xs text-innoq-apricot mb-4 font-mono">{mode.warning}</p>
                                    )}
                                    <button
                        ...
                        {success && (
                            <div className="flex items-start gap-2 text-innoq-red bg-innoq-red/10 p-4 rounded-lg">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-innoq-red mt-1">
                        ...
                        {agentActions.length > 0 && (
                            <div className="space-y-4">
                                <h3 className="text-lg font-medium flex items-center gap-2">
                                    <Bot className="w-5 h-5 text-innoq-red" />
                                    Agent Actions Log
                                </h3>
                                <div className="bg-white rounded-lg divide-y divide-innoq-gray-25">
                                    {agentActions.map((action, index) => (
                                        <div key={index} className="p-4 flex items-start gap-3">
                                            {action.type === 'read' && <Mail className="w-5 h-5 text-innoq-blue" />}
                                            {action.type === 'send' && <Send className="w-5 h-5 text-innoq-green" />}
                                            {action.type === 'delete' && <AlertTriangle className="w-5 h-5 text-innoq-red" />}
                                            {action.type === 'warning' && <FileWarning className="w-5 h-5 text-innoq-apricot" />}
                                            <div>
                                                <p className="font-medium text-innoq-gray-75">{action.action}</p>
                                                <p className="text-sm text-innoq-gray-75">{action.details}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                        ...
                        {response && (
                            <div className="space-y-4 mt-6">
                                <h3 className="text-lg font-mono font-medium" style={{ color: ACCENT_COLOR }}>
                                    <span className="text-innoq-gray-75">&gt;</span> Agent Response
                                </h3>
                                <div className="bg-white p-4 rounded-lg border" style={{ borderColor: `${ACCENT_COLOR}33` }}>
                                    <p className="text-innoq-gray whitespace-pre-wrap font-mono text-sm">{response}</p>
                                </div>
                            </div>
                        )}
                    </div>
